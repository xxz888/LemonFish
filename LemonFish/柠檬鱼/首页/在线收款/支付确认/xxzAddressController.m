...
#import "xxzUpdateView.h"
#import "xxzPrefixView.h"

@interface xxzAddressController ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate>
@property (nonatomic, strong) xxzUpdateView *shoukkuanblock;
...
@implementation xxzAddressController
...
- (void)viewDidLoad {
    ...
    NSDictionary *huabeiblock7 = [self loadUserData];
...
- (xxzUpdateView *)shoukkuanblock
{
    if (!_shoukkuanblock) {
        _shoukkuanblock = [[xxzUpdateView alloc] init];
    }
...
- (void)TheOrderStatus{
    @weakify(self);
    NSMutableDictionary *kehushuE = [NSMutableDictionary dictionary];
    [kehushuE setValue:self.startDic[@"id"] forKey:@"channelId"];
    [kehushuE setValue:self.huankuanjine forKey:@"orderCode"];
    NSDictionary *huabeiblockZ = [self loadUserData];
    [kehushuE setValue:huabeiblockZ[@"idcard"] forKey:@"idCard"];
    
    [self NetWorkingPostWithURL:self hiddenHUD:NO url:@"/api/payment/fast/query" Params:kehushuE success:^(id  _Nonnull responseObject) {
        
        xxzPrefixView *jihuajine = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([xxzPrefixView class]) owner:nil options:nil] lastObject];
...
-(void)fastApply{
    __weak typeof(self) weakSelf = self;
    NSDictionary *huabeiblock6 = [self loadUserData];
    ...
    [pass setValue:huabeiblock6[@"username"] forKey:@"loginPhone"];
    [pass setValue:huabeiblock6[@"idcard"] forKey:@"idCard"];
